...
class Attributes: 
    ...
    def get_copy(self):
        return Attributes(precision = self.precision, interval = self.interval, debug = self.debug, exact = self.exact, tag = self.tag, max_abs_error = self.max_abs_error, silent = self.silent, handle = self.handle, clearprevious = self.clearprevious, rounding_mode = self.rounding_mode)

    def get_light_copy(self):
        return Attributes(precision = self.precision, debug = self.debug, tag = self.tag, silent = self.silent, handle = self.handle, clearprevious = self.clearprevious, rounding_mode = self.rounding_mode)
