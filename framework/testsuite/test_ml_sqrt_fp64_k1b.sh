...
ML_DIR=/work1/nbrunie/tools_master/metalibm
 ...
 &&  python $ML_DIR/framework/testbench/gen_func_test_gen.py --round-mode $RND_MODE --informat $INPUT_FORMAT --outformat $OUTPUT_FORMAT --output func_test_gen.cpp --mpfr mpfr_sqrt --result-extract ./result_extract.c  \
 &&  echo -e "\033[32;1m Building and executing test vector generator \033[0m" \
 &&  make -f $ML_DIR/framework/testbench/test_lib/Makefile gen_test MPFR_LIB=-lmpfr TEST_LIB_DIR=$ML_DIR/framework/testbench/test_lib SUPPORT_LIB_DIR=$ML_DIR/framework MPFR_INCLUDE_DIR=/work1/nbrunie/local_install/bin TEST_FUNCTION=new_sqrt NUM_TEST=$NUM_TEST RESULT_EXTRACT_FILE=$RESULT_EXTRACT_FILE INF_INTERVAL=-1.0f SUP_INTERVAL=0x1.0p1023 SEED=$SEED RESULT_EXTRACT_DIR=./ \
 &&  echo -e "\033[32;1m Generating testbench \033[0m" \
 &&  python $ML_DIR/framework/testbench/gen_test_template.py --test-mode cr --informat $INPUT_FORMAT --outformat $OUTPUT_FORMAT --output $TEST_TEMPLATE_FILE --test new_sqrt \
 &&  echo -e "\033[32;1m Generating function implementation \033[0m" \
 &&  python $ML_DIR/framework/ml_functions/ml_sqrt_clean.py --target k1b --output new_sqrt.c $EXTRA_GEN_OPTIONS --fname new_sqrt --precision binary64 \
 &&  echo -e "\033[32;1m Building function implementation within testbench \033[0m" \
 &&  make -f $ML_DIR/framework/testbench/test_lib/Makefile build_test HAL_INCLUDE=/work1/nbrunie/tools_multiarch_bare_build/HAL/machine/common/core/  TEST_LIB_DIR=$ML_DIR/framework/testbench/test_lib SUPPORT_LIB_DIR=$ML_DIR/framework TEST_FUNCTION=new_sqrt NUM_TEST=$NUM_TEST FUNCTION_FILE=new_sqrt.c TEST_START=$TEST_START TEST_END=9999 TEST_TEMPLATE_FILE=$TEST_TEMPLATE_FILE BUILD_CC=$SPECIFIC_CC DEBUG_FLAG=-DML_DEBUG KML_MODE=-DKML_MODE  EXTRA_CC_OPTION=-march=k1b\
