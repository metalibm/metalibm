...
class StaticVectorizer(object):
  ...
  def vectorize_scheme(self, optree, arg_list, vector_size, call_externalizer, output_precision):
    ...
    arg_list_copy = dict((arg_node, Variable("vec_%s" % arg_node.get_tag(), arg_node.get_precision())) for arg_node in arg_list)
  ...
  def is_vectorizable(optree):
    return isinstance(optree, ML_ArithmeticOperation)
  ...
  def vector_replicate_scheme_in_place(self, optree, vector_size, _memoization_map = None):
    ...
    if optree in memoization_map:
      ...
      if self.is_vectorizable(optree):
        optree.set_precision(self.vectorize_format(optree.get_precision))
        for optree_input in self.get_inputs():
